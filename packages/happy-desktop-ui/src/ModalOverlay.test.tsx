...
it("holds ModalOverlay backdrop geometry, centering, and backdrop-only dismiss", async () => {
    ...
    expect(overlay.element.getAttribute("data-happy-desktop-ui")).toBe("modal-overlay");
    expect(
...
it("keeps explicit center byte-identical to the attribute-free default placement", async () => {
    ...
    expect(
        view.$('[data-testid="explicit"] [data-happy-desktop-ui="modal-dialog"]').bounds(),
...
it("anchors top placement in a constrained 720x480 host and dismisses from real backdrop coordinates", async () => {
    ...
    const layout = view.$(
        '[data-testid="top-minimum"] [data-happy-desktop-ui="modal-overlay-top-layout"]',
    );
...
it("anchors top placement at the 1024x704 design reference", async () => {
    ...
    const layout = view.$(
        '[data-testid="top-reference"] [data-happy-desktop-ui="modal-overlay-top-layout"]',
    );
    const card = view.$('[data-testid="top-card"]');
    expect(overlay.element.getAttribute("data-placement")).toBe("top");
...
it("moves focus into the dialog on open, closes on Escape without a click, and restores the opener", async () => {
    ...
    const overlay = view.container.querySelector('[data-happy-desktop-ui="modal-overlay"]')!;
    expect(overlay.contains(document.activeElement)).toBe(true);
    await userEvent.keyboard("{Escape}");
    expect(dismissed).toBe(1);
    expect(view.container.querySelector('[data-happy-desktop-ui="modal-overlay"]')).toBeNull();
